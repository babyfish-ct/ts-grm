...
export type __DslReferenceKeyMembers<TModel extends AnyModel, TMembers, TNullity extends __NullityType> = {
    [
        K in keyof TMembers as
            TMembers[K] extends __ReferencePropContract<infer _, any, "COLUMNS", never, any, infer TKey>
                ? TKey extends string
                    ? `${K & string}${Capitalize<__RequiredModelKey<TModel, TKey>>}`
                    : never
                : never
    ]: TMembers[K] extends __ReferencePropContract<infer TargetModel, infer Nullity, "COLUMNS", never, any, infer Key>
        ? Key extends string
